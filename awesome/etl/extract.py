"""Extraction logic for the Ecosyste.ms Awesome dataset."""
...
from awesome.api import AwesomeAPI
from awesome.etl import ExtractInterface
...
class AwesomeExtract(ExtractInterface):
    ...
    def __init__(
        ...
        email : str
            Contact email sent to the Awesome API via ``mailto``.
    ...
    def get_mentions(self) -> None:
        ...
        with Bar(
            "Getting project mentions from Ecosyste.ms Awesome API...",
    ...
    def download_data(self) -> list[dict[str, list[dict]]]:
        """
        Download all projects and mentions from the Awesome API.
        ...
        self.get_lists()
        # Disabled: AwesomeAPI has no get_mentions_from_project method and
        # `self.projects` is never populated (no get_projects step exists).
        # self.get_mentions()
